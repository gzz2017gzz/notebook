...
public class Application {
	...
	public static void main(String[] args) throws IOException {
		InputStream resourceAsStream = Application.class.getClassLoader().getResourceAsStream("no.jpg");
		bi = ImageIO.read(resourceAsStream);
		图片分类解析();
	}

	private static void 图片分类解析() throws IOException {
		Document 首页面 = Jsoup.connect("https://www.meitulu.com/t/imiss/").get();
		...
		for (Element 单个分类元素 : 所有分类) {
			...
			System.out.println("*************************************" + 单个分类元素.childNode(0).toString());
		}

	}

	private static void 解析页数(String 单类地址) throws IOException {
	...
	private static void 解析每页图集(String url) throws IOException {
	...
	public static final byte[] 输入流转字节数组(InputStream inStream) throws IOException {

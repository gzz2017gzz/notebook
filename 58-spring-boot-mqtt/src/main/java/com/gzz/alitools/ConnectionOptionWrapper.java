...
public class ConnectionOptionWrapper {
	...
	/**
	 * MQ4IOT clientId，由业务系统分配，需要保证每个 tcp 连接都不一样，保证全局唯一，如果不同的客户端对象（tcp 连接）使用了相同的
	 * clientId 会导致连接异常断开。 clientId 由两部分组成，格式为 GroupID@@@DeviceId，其中 groupId 在
	 * MQ4IOT 控制台申请，DeviceId 由业务方自己设置，clientId 总长度不得超过64个字符。
	 */
	private String clientId;
	/**
	 * 客户端使用的 Token 参数，仅在 Token 鉴权模式下需要设置，Key 为 token 类型，一个客户端最多存在三种类型，R，W，RW，Value
	 * 是 token内容。 应用需要保证 token 在过期及时更新。否则会导致连接异常。
	 */
	...
	public ConnectionOptionWrapper(String instanceId, String accessKey, String secretKey, String clientId) throws NoSuchAlgorithmException, InvalidKeyException {
		...
		mqttConnectOptions.setMqttVersion(MQTT_VERSION_3_1_1);
		mqttConnectOptions.setConnectionTimeout(5000);
		mqttConnectOptions.setServerURIs(new String[] { "tcp://post-cn-4591guvw90s.mqtt.aliyuncs.com:1883" });
	}

...
import com.gzz.repository.BlogRepository;
...
public class Crawler {
	...
	private Elements getElements(String url, String css) {
		...
		try {
			document = Jsoup.connect(url).data("query", "Java").userAgent("Mozilla").cookie("auth", "token").timeout(3000).post();
		} catch (IOException e) {
